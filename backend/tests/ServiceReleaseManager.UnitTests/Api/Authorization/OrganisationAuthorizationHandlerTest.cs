...
public class OrganisationAuthorizationHandlerTest : OrganisationAuthorizationHandler
{
  ...
  public async Task HandleRequirementAsync_Should()
  {
    ...
    var req = new OrganisationAuthorizationRequirement() { Name = "userReq", EvaluationFunction = r => r.UserWrite && r.UserRead && r.UserDelete };
    ...
    var authContextMock_UserAdmin = new Mock<AuthorizationHandlerContext>(Enumerable.Empty<IAuthorizationRequirement>().Append(req), new ClaimsPrincipal(), roleUserAdmin);
