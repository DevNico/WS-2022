...
public class ReleaseServiceTest
{
  ...
  public async Task TestReleaseServiceAsync()
  {
    ...
    _serviceRepositoryMock
     .Setup(m => m.CountAsync(It.IsAny<ServiceByIdSpec>(), It.IsAny<CancellationToken>()))
     .ReturnsAsync(1);

    _repositoryMock
     .Setup(m => m.ListAsync(It.IsAny<ReleaseByServiceIdSpec>(), It.IsAny<CancellationToken>()))
     .ReturnsAsync(new List<Release> { release0, release1 });
    ...
    _repositoryMock.Verify(
      m => m.ListAsync(
        It.IsAny<ReleaseByServiceIdSpec>(),
        It.Is<CancellationToken>(c => c == cancellationToken)
      ),
      Times.Once
    );
