...
public class OrganisationRoleServiceTest
{
  ...
  private readonly Mock<IRepository<OrganisationRole>> _roleServiceMock;
  private readonly Mock<IRepository<OrganisationUser>> _userServiceMock;
  ...
  public async Task CreateShould()
  {
    ...
    _roleServiceMock
     .Setup(m => m.AddAsync(It.IsAny<OrganisationRole>(), It.IsAny<CancellationToken>()))
     .ReturnsAsync(role);

    var organisationRoleService = new OrganisationRoleService(
  ...
  public async Task DeleteShould()
  {
    ...
    var organisation =
      new Organisation("test") { Id = orgId, Roles = new List<OrganisationRole> { role } };

    var cancellationToken = new CancellationToken();

    _userServiceMock
     .Setup(
        m => m.CountAsync(It.IsAny<OrganisationUsersByRoleIdSpec>(), It.IsAny<CancellationToken>())
      ).ReturnsAsync(0);
    ...
    _roleServiceMock
     .Setup(
        m => m.GetBySpecAsync(It.IsAny<OrganisationRoleByIdSpec>(), It.IsAny<CancellationToken>())
      ).ReturnsAsync(role);
    ...
    _userServiceMock.Verify(
      m => m.CountAsync(
        It.IsAny<OrganisationUsersByRoleIdSpec>(),
        It.Is<CancellationToken>(c => c == cancellationToken)
      ),
      Times.Once
    );
    ...
    _roleServiceMock.Verify(
      m => m.GetBySpecAsync(
        It.IsAny<OrganisationRoleByIdSpec>(),
        It.Is<CancellationToken>(c => c == cancellationToken)
      ),
      Times.Once
    );
  ...
  public async Task ListByOrganisationRouteNameShould()
  {
    ...
    _organisationRepositoryMock
     .Setup(
        m => m.GetBySpecAsync<IEnumerable<OrganisationRole>>(
          It.IsAny<OrganisationRolesByOrganisationRouteNameSpec>(),
          It.IsAny<CancellationToken>()
        )
      ).ReturnsAsync(organisation.Roles);
    ...
    var result =
      await organisationRoleService.ListByOrganisationRouteName(orgName, cancellationToken);
